...
/**
 * Created by IntelliJ IDEA. User: Ruairi Date: 08/11/12 Time: 19:00
 */
public class PlayBillingService extends Service implements ServiceConnection {
	...
	/**
	 * The list of requests that we have sent to Android Market but for which we have not yet received a response code. The
	 * HashMap is indexed by the request Id that each request receives when it executes.
	 */
	private static HashMap<Long, BillingRequest> mSentRequests =
			new HashMap<Long, BillingRequest>();

	/**
	 * The base class for all requests that use the MarketBillingService. Each derived class overrides the run() method to
	 * call the appropriate service interface.  If we are already connected to the MarketBillingService, then we call the
	 * run() method directly. Otherwise, we bind to the service and save the request on a queue to be run later when the
	...
	abstract class BillingRequest {
		 ...
		 *
		 * @return true if the request ran successfully; false if the service is not connected or there was an error when
		 *         trying to use it
		 */
		...
		/**
		 * Called when a remote exception occurs while trying to execute the {@link #run()} method.  The derived class can
		 * override this to execute exception-handling code.
		 *
	 ...
	 * <p/>
	 * Developers may want to perform two checks if both one-time and subscription products are available.
	 */
	class CheckBillingSupported extends BillingRequest {
		 ...
		 * Legacy contrustor
		 * <p/>
		 * This constructor is provided for legacy purposes. Assumes the calling application will not be using any features
		 * not present in API v1, such as subscriptions.
		 */
		@Deprecated
		 ...
		 * Developers may want to perform two checks if both one-time and subscription products are available.
		 *
		 * @param itemType Either Consts.ITEM_TYPE_INAPP or Consts.ITEM_TYPE_SUBSCRIPTION, indicating the type of item support
	...
	class RequestPurchase extends BillingRequest {
		 ...
		 * Constructor
		 *
		 * @param itemId           The ID of the item to be purchased. Will be assumed to be a one-time purchase.
		 * @param itemType         Either Consts.ITEM_TYPE_INAPP or Consts.ITEM_TYPE_SUBSCRIPTION, indicating the type of item
		...
		@Override
		protected void responseCodeReceived(Consts.ResponseCode responseCode) {
			ResponseHandler.responseCodeReceived(PlayBillingService.this, this, responseCode);
		}
	}
	...
	/**
	 * The {@link com.trust5.billing.library.googleplay.PlayBillingReceiver} sends messages to this service using intents.
	 * Each intent has an action and some extra arguments specific to that action.
	 *
	 * @param intent  the intent containing one of the supported actions
	 * @param startId an identifier for the invocation instance of this service
	 */
	void handleCommand(Intent intent, int startId) {
	 ...
	 * @return true if supported; false otherwise
	 */
	boolean checkBillingSupported(String itemType) {
	 ...
	 * {@link PlayBillingReceiver} receives an intent with the action {@link Consts#ACTION_NOTIFY}. Returns false if there
	 ...
	 */
	boolean requestPurchase(String productId, String itemType, String developerPayload) {
	 ...
	 * server. This method sends those identifiers back to the MarketBillingService, which ACKs them to the server. Returns
	 * false if there was an error trying to connect to the MarketBillingService.
	 *
	 * @param startId   an identifier for the invocation instance of this service
	...
	/**
	 * Gets the purchase information. This message includes a list of notification IDs sent to us by Android Market, which
	 * we include in our request. The server responds with the purchase information, encoded as a JSON string, and sends
	 * that to the {@link com.trust5.billing.library.googleplay.PlayBillingReceiver} in an intent with the action {@link
	 * Consts#ACTION_PURCHASE_STATE_CHANGED}. Returns false if there was an error trying to connect to the
	...
	/**
	 * Verifies that the data was signed with the given signature, and calls {@link ResponseHandler#purchaseResponse(Context,
	 * Consts.PurchaseState, String, String, long, String)} for each verified purchase.
	 *
	 * @param startId    an identifier for the invocation instance of this service
	 ...
	 * purchase state changes.  All purchase state changes are received in the {@link
	 * com.trust5.billing.library.googleplay.PlayBillingReceiver} and passed to this service, where they are handled in
	 * {@link #purchaseStateChanged(int, String, String)}.
	 *
	 * @param requestId    a number that identifies a request, assigned at the time the request was made to Android Market
	 * @param responseCode a response code from Android Market to indicate the state of the request
	 */
	...
	/**
	 * Runs any pending requests that are waiting for a connection to the service to be established.  This runs in the main
	 * UI thread.
	 */
