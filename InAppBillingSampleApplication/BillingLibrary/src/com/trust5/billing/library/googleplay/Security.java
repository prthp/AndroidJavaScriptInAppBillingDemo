...
import com.trust5.billing.library.util.Base64;
import com.trust5.billing.library.util.Base64DecoderException;
 ...
 * communicates with the application on the device. For the sake of simplicity and clarity of this trust5, this code is
...
class Security {
	 ...
	 * a new nonce. This has to be "static" so that the {@link PlayBillingReceiver} can check if a nonce exists.
	 */
	 ...
	 * whole data string) with a private key. The data also contains the {@link com.trust5.billing.library.googleplay.Consts.PurchaseState}
	 * and product ID of the purchase. In the general case, there can be an array of purchase transactions because there
	 * may be delays in processing the purchase on the backend and then several purchases can be batched together.
	 *
	 * @param signedData the signed JSON string (signed, not encrypted)
	 * @param signature  the signature for the data, signed with the private key
	 */
	static ArrayList<VerifiedPurchase> verifyPurchase(String signedData, String signature) {
		if (signedData == null) {
		...
		if (!TextUtils.isEmpty(signature)) {
			 ...
			 * use bit manipulation (for trust5, XOR with some other string) to hide
